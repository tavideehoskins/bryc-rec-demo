...
| **Content modules — source** *(2026-07-09)* | The "Making It Pay Off" copy + universal on-time actions now live in a Notion database: **🧭 Rules of the Game** in *BRYC App — Knowledge Base* (`collection://ae5402d3-dbe9-436a-ba34-ee65aa415fc0`), one record per **Key** (`terminal-definition` · `transfer-risk` · `on-time-actions` · `transfer-associate` · `aas` · `certificate` · `apprenticeship`). Engine resolves the program's `:rules-type` / terminal-transfer flag → Key → reads the record body. Advisor-editable (no code deploy). Prototype `data.cljs` mirrors it (`rules-source` holds the DB + per-Key record URLs). |
