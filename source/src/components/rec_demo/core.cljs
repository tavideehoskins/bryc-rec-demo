...
(defui salary-section [{:keys [pathway]}]
  (let [s (:salary pathway)
    ...
    ($ :div {:class "space-y-5"}
       ...
       ($ :div {:class "rounded-xl bg-[#d0ecef]/50 p-4 text-sm md:text-base text-[#313335] leading-relaxed"}
          ...
          " a living wage in Baton Rouge.")
       ($ :div {:class "grid grid-cols-1 md:grid-cols-3 gap-3"}
          (for [[i t] (map-indexed vector (:tiles s))]
...
(defui pums-subsection [{:keys [pums]}]
  ($ :div {:class "bg-[#f2f3f4] rounded-xl p-4 space-y-3"}
     ($ :div
        ...
        ($ :p {:class "text-xs text-[#676868]"}
           (str "Where " (:scope pums) " nursing-degree holders actually work "
          ...
          {:bullets
           [($ :span nil
               ...
               ($ ext-link {:href (:campus-life-url school) :label "campus life"
                            :title "Southeastern — campus life"})
               ".")
...
(defui costs-section [{:keys [school]}]
  (let [c (:costs school)
    ...
    ($ :div {:class "space-y-4"}
       ...
       ($ ext-link {:href (:npc-url c) :label "See Southeastern's cost & financial aid info →"
                    :title "Southeastern — Cost & Financial Aid"}))))
    ...
    ($ :div {:class "bg-white rounded-xl shadow-sm ring-1 ring-[#bcdfe5] overflow-hidden"}
       ($ :button {:class "w-full flex items-center justify-between gap-3 p-4 text-left hover:bg-[#f3fafb] transition-colors"
          ...
          ($ :div {:class "min-w-0"}
             ($ :div {:class "font-semibold text-[#2a6465] font-head"}
                (str (:name pathway) " (" (:acronym pathway) ")"))
       ...
       (when open?
         ($ :div {:class "px-3 md:px-4 pb-4 pt-2 bg-[#f2f3f4]"}
            ($ pathway-sections {:pathway pathway}))))))

(defui school-sections
  "School-anchored sections — rendered ONCE per school, shared across all pathways."
  [{:keys [school student]}]
  ;; Default-open + anchored under the pathways (collapsible, but shown unfurled).
...
(defui school-expanded
  "What unfurls under a school tile: the recommended pathways (each unfurls its own
   1–4 sections) followed by the school-anchored About This School + What It Costs You."
  [{:keys [school student]}]
  ($ :div {:class "space-y-5"}
     ($ :div {:class "space-y-3"}
        ($ section-eyebrow {:label (str (count data/pathways) " Recommended Pathway"
                                        (when (> (count data/pathways) 1) "s"))})
        ;; Pathways start COLLAPSED — the student clicks one to unfurl its details.
        (for [p data/pathways]
          ($ pathway-row {:key (:id p) :pathway p :default-open? false})))
  ...
  (let [[open? set-open!] (use-state false)
    ...
    ($ card/Card {:class "bg-white rounded-2xl shadow-sm ring-1 ring-[#d0ecef] border-0 overflow-hidden"}
       ($ :div {:class "p-5 space-y-3"}
          ...
          ;; -ml-2.5 cancels the first pill's left padding so the tag TEXT lines up
          ;; with the city name above it.
          ($ :div {:class "flex flex-wrap gap-2 -ml-2.5"}
             (for [t (:tags (first data/pathways))]
       ...
       ($ :div {:class "max-w-6xl mx-auto px-4 md:px-8 py-8"}
          ($ :div {:class "grid grid-cols-1 lg:grid-cols-12 gap-6 items-start"}
             ...
             ($ :div {:class "lg:col-span-8"}
                ($ recommendations-column {:school data/school :student s})))))))
